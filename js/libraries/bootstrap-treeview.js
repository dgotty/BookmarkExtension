...
;(function($, window, document, undefined) {
    ...
    Tree.prototype = {
        ...
        _toggleNodes: function(node) {
            ...
            if (!node.nodes && !node._nodes && !node.children) {
                return;
            }
            ...
            if (node.nodes) {
                ...
                node._nodes = node.children;
                delete node.children;
            } else {
                node.nodes = node._nodes;
                node.children = node._nodes;
                delete node._nodes;
            }
        },
        ...
        _buildTree: function(nodes, level) {
            ...
            $.each(nodes, function addNodes(id, node) {
                ...
                var nodeText = node.text || node.title;
                var children = node.nodes || node.children;
                ...
                if (children) {
                    return self._buildTree(children, level);
                }
            });
        },
